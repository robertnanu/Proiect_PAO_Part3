...
public class DealerShip implements Comparable<DealerShip>, DATA {
    ...
    public void readFromDatabase(String name_table) throws SQLException {
        ...
        ResultSet res = db.query("SELECT name FROM " + name_table + " WHERE dealershipId = " + dealershipId);
        res.next();
    ...
    public void writeToDatabse(String name_table) throws SQLException {
        DB db = DB.getInstance();
        db.reload("INSERT INTO " + name_table + "(dealershipId, name) VALUES " + "(" + dealershipId + ",'" + name + "')");
    }
}
